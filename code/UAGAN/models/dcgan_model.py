...
class DCGANModel(BaseModel):
    ...
    def backward_D(self):
        ...
        label_img = self.fill[self.real_A].to(self.device)
        pred_fake = self.netD(self.fake_B.detach(), label_img)  # we use label_img as condition
        self.loss_D_fake = self.criterionGAN(pred_fake, False)
        # Real
        self.real_B.requires_grad_()
        pred_real = self.netD(self.real_B, label_img)  # label as conditional image
    ...
    def backward_G(self):
        ...
        label_img = self.fill[self.real_A].to(self.device)
        pred_fake = self.netD(self.fake_B, label_img)     # label as onehot encoding in G
