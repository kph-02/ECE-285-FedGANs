...
class UAGANModel(BaseModel):
    ...
    def backward_D(self):
        ...
        for i in range(len(self.real_A)):
            ...
            pred_fake = self.netD[i](self.fake_B.detach(), label_img_fake)
    ...
    def backward_G(self):
        ...
        for i in range(len(self.real_A)):
            ...
            if pred_fake_weighted is None:
                pred_fake_weighted = pred_fake / (1 - pred_fake + 1e-8) * self.weights[i][self.rand_label].unsqueeze(1).unsqueeze(2).unsqueeze(3)
            else:
                pred_fake_weighted += pred_fake / (1 - pred_fake + 1e-8) * self.weights[i][self.rand_label].unsqueeze(1).unsqueeze(2).unsqueeze(3)
        # our aggregation
